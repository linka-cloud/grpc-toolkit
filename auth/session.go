...
type Handler[T TokenResponse, U proto.Message] interface {
...
func NewHandler[T TokenResponse, U proto.Message](sessionName string, sessions *sessions.CookieStore) Handler[T, U] {
	return &handler[T, U]{sessionName: sessionName, sessions: sessions}
}

type handler[T TokenResponse, U proto.Message] struct {
	sessionName string
	sessions    *sessions.CookieStore
}

// ForwardTokenAsCookie forwards the token from the message to the cookie store.
func (h *handler[T, U]) ForwardTokenAsCookie(_ context.Context, w http.ResponseWriter, p proto.Message) error {
	switch m := any(p).(type) {
	case T:
...
func (h *handler[T, U]) ForwardCookieAsToken(_ context.Context, request *http.Request) metadata.MD {
...
func (h *handler[T, U]) SendAuthCookie(ctx context.Context, token string) error {
	sess := h.newSession(h.sessionName)
...
func (h *handler[T, U]) newSession(name string) *sessions.Session {
...
func (h *handler[T, U]) CookieToAuth(ctx context.Context) context.Context {
